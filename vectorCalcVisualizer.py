...
table = tabulate(l, headers=['Function/Operation', 'Description', 'Code'], tablefmt='fancy_grid')
...
def formatFunction(strF):
  ...
  for x in i:
      ...
      for y in range(0, len(x)):
          x += ' '
          #print(x[y])
          if(x[y].isnumeric() & x[y+1].isalpha()):
              eval1 += x[y] + '*' + x[y+1:]
            #print(x)
      #print('ev: ' + eval)
      newL.append(eval1[0:len(eval1) - 1])
  return newL
...
def drawVector(vecL):
   ...
   mag = magSum **(0.5)
   l2 = [["Yellow", mag]]
   table2 = tabulate(l2, headers=['Vector', 'Magnitude'], tablefmt='fancy_grid')
...
def vectorFx():
  ...
  l3 = [["x(t)", "3t^2, 5t + 1, 4t"], ["v(t)", "5t^2, 6t, 4"], ["a(t)", "5t, 2t, 3"]]
  table3 = tabulate(l3, headers=['Type', 'Example Input'], tablefmt='fancy_grid')
  funcType = input("Is your function for position, velocity, or acceleration?  Please enter  x , v, or a  to indicate")
  print(table3)
